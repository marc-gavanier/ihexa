...
import { SEARCH_COMPANY_KEY } from '../../action/create-client.key';
...
const loadSuggestions = async (inputValue: string): Promise<{ items: CompanySummary[] }> => {
  if (inputValue.length < MIN_COMPANY_QUERY_LENGTH) return { items: [] };
...
export const companyOptions: {
  itemToKey: typeof itemToKey;
  renderItem: (props: { item: CompanySummary }) => ReactNode;
} = {
  itemToKey,
  renderItem: ({ item }) => (
