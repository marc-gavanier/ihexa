...
import { type AggregateRoot, type DomainError, toAggregateRoot, toValueObject } from '@/libraries/ddd';
...
export type InvalidInvoiceError = InvalidInvoiceIdError | InvalidRecipientError | InvalidLineError;
...
const toInvoiceTotal = (total: Effect<Amount, InvalidAmountError>, line: Line): Effect<Amount, InvalidAmountError> =>
  flatMap(total, (amount: Amount) => Amount(amount + line.total));

const toLines = (lines: [Line, ...Line[]]): Lines => toValueObject(lines);

const totalFrom = (lines: Lines) => lines.reduce(toInvoiceTotal, Amount(0));

export const Invoice = (
  invoiceIdEffect: Effect<InvoiceId, InvalidInvoiceIdError>,
  linesEffects: [Effect<Line, InvalidLineError>, ...Effect<Line, InvalidLineError>[]],
  ...
  pipe(
    ...
    flatMap((lines: Lines) =>
      pipe(
        ...
        map(([id, recipient, total]) => toAggregateRoot({ id, lines, recipient, total }))
      )
    )
  );

export type InvoiceByIdNotFoundError = DomainError<'InvoiceByIdNotFoundError', never>;
