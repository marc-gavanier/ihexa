...
const FORBIDDEN_FEATURES = {
  ...
  ABILITIES: [
    ...
    },
    {
      ...
      comment:
        "Mutations and queries represent the fundamental operations of an ability. A mutation performs a specific business action that may change state, while a query retrieves information without side effects. Each mutation or query within an ability must remain fully isolated. They live under `src/features/<feature>/abilities/<ability>/mutation` or `query` and represent the smallest self-contained business actions or reads. Mutations and queries can only depend on the feature's domain, shared implementations, adapter implementations through dependency injection, shared libraries, configuration, and their own validation schemas.",
      severity: 'error',
      from: {
        path: ['^src/features/([^/]+)/abilities/([^/]+)/(?:mutation|query)/'],
        pathNot: '[.](?:spec|test|e2e|feature|stories)[.](?:js|mjs|cjs|jsx|ts|mts|cts|tsx)$'
      },
      to: {
        pathNot: [
          ...
          '^src/features/$1/abilities/$2/(?:mutation|query)/',
    ...
    },
    {
      ...
      comment:
        'Shared UI components within an ability can be imported by other abilities in the same feature, but they can only depend on their own ability, the feature domain, injection, feature-level UI components, and shared libraries. They cannot import from other abilities or other shared folders.',
      severity: 'error',
      from: {
    ...
    },
    {
      ...
      comment:
        "UI for an ability can depend on its own components, the feature's domain, shared libraries, validation schemas, and shared UI components from other abilities within the same feature.",
      severity: 'error',
      from: {
  ...
  DOMAIN: [
    {
      ...
      to: {
        pathNot: ['^node_modules/[^/]+/effect', '^src/features/$1/domain', '^src/libraries/effect']
      }
    }
  ],
  IMPLEMENTATIONS: [
    {
      ...
      comment:
        "Feature implementations provide concrete realizations of secondary adapters or technical details necessary for the feature to function. They can only depend on the feature's domain, feature's db, npm packages, configuration, or environment. No other source dependencies are allowed. This ensures that implementations remain focused on bridging the domain with technical concerns.",
      severity: 'error',
      from: {
...
const LIBRARIES_WITH_FEATURES_ACCESS = {
  i18n: ['^src/features/.*/i18n[.]ts$'] // i18n namespace files
};
...
const FORBIDDEN_LIBRARIES = [
  ...
  },
  {
    ...
    comment: `Only specific libraries can access env.ts: ${LIBRARIES_WITH_ENV_ACCESS.join(', ')}`,
    severity: 'error',
    from: {
      path: ['^src/libraries'],
